...
func (d *ServiceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Получает информацию о конкретной услуге Selectel Bare Metal по UUID или имени.",
		Description:         "Получает информацию о конкретной услуге Selectel Bare Metal по UUID или имени.",
		...
		Attributes: map[string]schema.Attribute{
			...
			"uuid": schema.StringAttribute{
				MarkdownDescription: "UUID услуги. Взаимоисключающий с `name`.",
				Description:         "UUID услуги.",
				Optional:            true,
				Computed:            true,
			},
			"name": schema.StringAttribute{
				MarkdownDescription: "Название услуги. Взаимоисключающий с `uuid`.",
...
func (d *ServiceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !data.UUID.IsNull() && !data.UUID.IsUnknown() {
		...
		tflog.Debug(ctx, "Reading service by UUID", map[string]interface{}{
			"uuid": uuid,
		})
		service, err = d.client.GetService(ctx, uuid)
	} else {
		// Get service by name
		...
		service, err = d.client.FindServiceByName(ctx, name)
	}
	...
	if service == nil {
		resp.Diagnostics.AddError("Service Not Found", "The specified service was not found.")
		return
	}

	// Map response body to model
	data.ID = types.StringValue(service.UUID)
	data.UUID = types.StringValue(service.UUID)
	data.Name = types.StringValue(service.Name)
	data.Type = types.StringValue(string(service.Type))
	data.Category = types.StringValue(service.Category)
	data.Description = types.StringValue(service.Description)
	data.Available = types.BoolValue(service.Available)
	data.LocationUUID = types.StringValue(service.LocationUUID)

	// Map CPU specs
	if service.CPU != nil {
		data.CPU = &CPUSpecModel{
	...
	if len(service.Storage) > 0 {
		data.Storage = make([]StorageSpecModel, len(service.Storage))
		for i, storage := range service.Storage {
			data.Storage[i] = StorageSpecModel{
	...
	if service.Network != nil {
		...
		for i, port := range service.Network.Ports {
			ports[i] = types.StringValue(port)
		}

		data.Network = &NetworkSpecModel{
	...
	if len(service.PricePlans) > 0 {
		data.PricePlans = make([]PricePlanModel, len(service.PricePlans))
		for i, plan := range service.PricePlans {
	...
	tflog.Debug(ctx, "Read service", map[string]interface{}{
		"uuid": service.UUID,
		"name": service.Name,
	})
